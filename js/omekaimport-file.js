function omeka_file_get(items, orig_length){
	if(items.length!=0){
		...
		var data = {
			'action': 'omeka_page_import',
			'id': jQuery(item).attr('id'),
			'url': jQuery(item).attr('url'),	
			'nonce': omeka_import_page.nonce
		};
		...
		jQuery.post(omeka_import_page.ajaxURL, data, function(response) {
		...
		children = Array();
		jQuery("form#omeka_choose_page_form")
			.children()
			.each(
...
jQuery(document).ready(
	function(){
		...
		jQuery("form#omeka_choose_page_form #omeka_page_submit")
			.on("click", 
					function(){
						...
						jQuery("form#omeka_choose_page_form input:checked")
							.each(							
								function(index,value){	
									items.push(value);									
								}
